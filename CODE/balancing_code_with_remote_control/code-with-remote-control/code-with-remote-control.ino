...
int acc_calibration_value = -410;                            //Enter the accelerometer calibration value
...
int battery_voltage;
int receive_counter, receive_counter1;
...
void setup(){
  ...
  pinMode(13, OUTPUT);     
  pinMode(11, INPUT);                                                       //Configure digital poort 5 as output
  pinMode(12, INPUT);                                                  //Configure digital poort 6 as output
...
void loop(){
  ...
  int d11_state = digitalRead(11);  // Read the state of pin D11
  int d12_state = digitalRead(12);  // Read the state of pin D12

  // Determine the received byte based on D11 and D12
  if (d11_state == LOW && d12_state == HIGH) {
    receive_counter1 = 0;  // Reset the receive_counter variable
    received_byte = B00000001;  // D11 = 0, D12 = 1
  } else if (d11_state == HIGH && d12_state == LOW) {
    receive_counter1 = 0;  // Reset the receive_counter variable
    received_byte = B00000010;  // D11 = 1, D12 = 0
  } else if (d11_state == HIGH && d12_state == HIGH) {
    receive_counter1 = 0;  // Reset the receive_counter variable
    received_byte = B00000100;  // D11 = 1, D12 = 1
  }
  else {
    received_byte = B00000000;  // D11 = 0, D12 = 0
    receive_counter1 = 0;  // Reset the receive_counter variable
  }

  if(receive_counter1 <= 25)receive_counter1 ++;                              //The received byte will be valid for 25 program loops (100 milliseconds)
  else received_byte = B00000000;                                                //After 100 milliseconds the received byte is deleted
  ...
  if(!(received_byte & B00001100)){                                         //Slowly reduce the setpoint to zero if no foreward or backward command is given
    if(pid_setpoint > 
    0.5)pid_setpoint -=0.05;                              //If the PID setpoint is larger then 0.5 reduce the setpoint with 0.05 every loop
