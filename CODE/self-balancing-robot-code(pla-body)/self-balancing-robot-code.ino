...
void loop()
{
  ...
  { // If the setpoint is zero degrees
    if (pid_output < 0)
      self_balance_pid_setpoint += 0.0010; // Increase the self_balance_pid_setpoint if the robot is still moving forewards
    if (pid_output > 0)
      self_balance_pid_setpoint -= 0.0010; // Decrease the self_balance_pid_setpoint if the robot is still moving backwards
  }
