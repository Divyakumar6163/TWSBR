...
const int dirPinRight = D5; // Another GPIO for the second motor's direction
const int stepPinRight = D0; // Another GPIO for the second motor's steps
...
void controlMotors(int motor_output_left, int motor_output_right) {
  ...
  int motorSpeedLeft = 500 + baseSpeed - abs(motor_output_left) * 5;
  int motorSpeedRight = 500 + baseSpeed - abs(motor_output_right) * 5;
  ...
  int xx=0;
  for(xx;xx<min(abs(motor_output_left),abs(motor_output_right));xx++){
